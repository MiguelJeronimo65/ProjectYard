...
1. **Fonte de verdade.** Este guia + `README_HANDOFF.md` + `handoff/db-schema-mysql.sql` mandam. A `Docs/ProjectYard_Master_Specification_v1.md` é **visão/roadmap** — não implementar à letra (ignorar: Clean Architecture de 6 projetos, Finbuckle, 2 BDs, Materio, Hangfire, ~80 tabelas e migrations dela).
...
> (`Pomelo.EntityFrameworkCore.MySql` **9.0.0** — Pomelo/EF Core 10 ainda não publicados; o stack EF 9.x corre no runtime .NET 10). Estrutura: projeto Web (MVC) + camada de dados.
...
**Comandos que ele deve correr (confirma):** `dotnet new mvc` (já cria com `net10.0`), `dotnet add package Pomelo.EntityFrameworkCore.MySql --version 9.0.0` (+ `Microsoft.EntityFrameworkCore.Design 9.0.0` no Web, p/ o scaffold da Fase 3).
**✓ Aceitação:** `dotnet build` passa; a app arranca em branco. *(Feito: solução `.slnx` com `ProjectYard.Web` + `ProjectYard.Data`.)*
...
Abre `handoff/db-schema-mysql.sql` no DBeaver, liga à base `db_aca76c_prjyard` e **executa o script** (cria as tabelas de domínio + as tabelas do Identity).
> ℹ️ **Identity:** a `users` é o store do Identity e as tabelas de papéis (`roles`, `user_roles`, …) são criadas **por SQL à mão** (decisão 2). Se já tinhas corrido uma versão anterior do `.sql`, aplica o upgrade incremental `handoff/db-identity-upgrade.sql` (acrescenta as colunas Identity à `users` + cria as tabelas de papéis) em vez de recriares tudo.
...
> (`dotnet ef dbcontext scaffold "Name=ConnectionStrings:DefaultConnection" Pomelo.EntityFrameworkCore.MySql -o Entities -c AppDbContext --project ProjectYard.Data --startup-project ProjectYard.Web`).
> **Exclui** do scaffold as tabelas do Identity (`users`, `roles`, `user_roles`, `role_claims`, `user_claims`, `user_logins`, `user_tokens`) — essas são definidas em código (`ApplicationUser : IdentityUser<long>`, `ApplicationRole : IdentityRole<long>`). Faz scaffold só às tabelas de **domínio**.
...
**✓ Aceitação:** as tabelas existem no MySQL (criadas pelo `.sql`); o `DbContext` de domínio (scaffold) e o de Identity compilam — **sem pasta `Migrations/`**.
...
> (`user_type=platform`) atravessa tenants COM registo de auditoria. (2) **Auth com ASP.NET Identity** usando a
> tabela `users` como store (`ApplicationUser : IdentityUser<long>`, `ToTable("users")`, chave BIGINT; mapeia as
> colunas Identity já criadas no `.sql`). As tabelas de papéis já existem (criadas por SQL) — **não geres migrations**.
> Distingue platform vs tenant e configura os papéis Owner/Administrador/Gestor/Membro/Cliente; autorização por
> `[Authorize(Roles=…)]` via `user_roles`. (3) Layout base (sidebar + topbar) a condizer com o protótipo
> (`prototype/ui.jsx`, `prototype/styles.css`).
